...
>**Note**: Because we previously created the certificate for the queue manager running on OpenShift, it was already available in the working directory. The alternative way is to obtain the certificate using *openssl*. First detect the host name from the queue manager's route: `export QMGR_HOST=$(oc get route qm2-ibm-mq-qm -o jsonpath="{.spec.host}")` Then extract and store the certificate:`openssl s_client -connect $QMGR_HOST:443 -servername $QMGR_HOST -showcerts | openssl x509 > queuemanager.crt`
